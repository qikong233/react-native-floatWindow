...
export default class extends PureComponent {
  ...
  componentWillMount () {
    this.state.translateValue.addListener(value => (this.listenerValue = value))
  ...
  onPanResponderMove = (evt, gestureState) => {
    ...
    const left = this.state.translateValue.x.__getValue()
    const top = this.state.translateValue.y.__getValue()
    this.isInArea(top, left)
  }

  onPanResponderRelease = (evt, gestureState) => {
    this.state.translateValue.flattenOffset()
    this.setState({ showDragArea: false }, () => this.hideDragArea())
  ...
  isInArea = (top, left) => {
    if (this.area) {
      if (top > this.area.top && left > this.area.left) {
  ...
  render () {
    ...
    return (
      <Animated.View style={[styles.container, animStyle, transformStyle]}>
        ...
        <View style={{ marginTop: 64, flex: 1, backgroundColor: 'white' }}>
          {this.props.children}
        </View>
        {this.renderFloatBtn()}
      </Animated.View>
